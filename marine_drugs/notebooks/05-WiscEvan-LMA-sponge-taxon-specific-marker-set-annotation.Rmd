...
ggsave(
  filename=here("marine_drugs/notebooks/05-WiscEvan-LMA-sponge-taxon-specific-marker-set-annotation_files/top-ten-taxa-by-sponge-type-and-sponge-shared-barplot.png"),
  p)
...
sponge_names <- read_tsv(here("~/marine_drugs/marine_drugs/data/raw/sponge_metadata.tsv")) %>% 
  mutate(taxon=if_else(str_detect(`Taxonomic Classification`, "/"),
                       # Neopetrosia proxima / Spheciospongia vesparium
                       str_replace(`Taxonomic Classification`,
                                   "(.{1}?).*\\s(.*?)\\s/\\s(.{1}?).*\\s(.*)",
