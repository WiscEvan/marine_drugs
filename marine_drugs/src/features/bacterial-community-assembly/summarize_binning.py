...
def main():
    ...
    parser.add_argument(
        "assembly",
        help="path to assemblies directory (will glob *.fasta)",
    )
    parser.add_argument(
        "out",
    ...
    binning_search_string = os.path.join(args.binning, "*.bacteria.binning.*.tsv")
    # We only want *.bacteria.binning.{umap,bhsne}.tsv, not bacteria.binning.hdbscan.tsv
    binning_files = [
        filepath for filepath in glob.glob(binning_search_string) if "hdbscan" not in filepath
    ]
    assemblies_search_string = os.path.join(args.assembly, "*.fasta")
    # We only want *.bacteria.binning.{umap,bhsne}.tsv, not bacteria.binning.hdbscan.tsv
    assemblies = [filepath for filepath in glob.glob(assemblies_search_string)]
    samples = []
